...
@section('js_after')
    ...
    <script>
        function loadFormNatural(){
            document.getElementById('name').placeholder = "{{__('generics.name')}}";
            document.getElementById('document').placeholder = "{{__('generics.document')}}";
            let type = document.getElementById('customer_type');
            type.setAttribute('value', 'Natural');
        }

        function loadFormJuridical(){
            document.getElementById('name').placeholder = "{{__('generics.business_name')}}";
            document.getElementById('document').placeholder = "{{__('generics.nit')}}";
            let type = document.getElementById('customer_type');
            type.setAttribute('value', 'Juridical');
        }
...
@section('content')
    ...
    <div class="hero-static d-flex align-items-center">
        ...
        <div class="w-100">
            ...
            <!-- Sign Up Section -->
            <div class="content content-full bg-white" style="background-color: #000000 !important; opacity: 70%">
                    ...
                    <div class="col-md-8 col-lg-6 col-xl-4 py-4">
                        ...
                        <div class="text-center">
                            ...
                            <h2 class="h6 font-w400 text-muted mb-3">
                                @lang('messages.create_acount_message')
                            </h2>
                        </div>
                        <!-- END Header -->
                        ...
                        <form class="js-validation-signup" method="POST" action="{{ route('customers.registers') }}" onsubmit="return acceptTyC()">
                            @csrf

                            <input id="customer_type" name="customer_type" type="hidden"  value="Natural">

                            <div class="form-group">
                                    <input id="name" type="text" placeholder="{{ __('generics.name') }}" class="form-control form-control-lg form-control-alt{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" required autofocus>
                            ...
                            <div class="form-group">
                                    <input id="document" type="hidden" placeholder="{{ __('generics.document') }}" class="form-control form-control-lg form-control-alt{{ $errors->has('document') ? ' is-invalid' : '' }}" name="document" value="{{ old('document') }}" required autofocus>
                            ...
                            <div class="form-group">
                                <input id="email" type="email" placeholder="{{ __('generics.email') }}" class="form-control form-control-lg form-control-alt{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required>
                            ...
                            <div class="form-group">
                                <div class="d-md-flex align-items-md-center justify-content-md-between">
                                    <div class="custom-control custom-checkbox">
                                        ...
                                        <label class="custom-control-label font-w400" for="signup-terms">@lang('messages.accept_T&C')</label>
                                    </div>
                                    <div class="py-2">
                                        <a class="font-size-sm text-smirrow" href="javascript:void(0)" data-toggle="modal" data-target="#one-signup-terms"><i class="fa fa fa-list-alt"> </i> @lang('messages.view_terms')</a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group row text-center pt-3">
                                {{-- <div class="col-md-6 ">
                                    <a href="{{ route('handleSocialLogin', ['customer', 'facebook']) }}" class="btn btn-outline-secondary btn-block"><i class="fab fa-facebook"></i> Facebook</a>

                                </div> --}}
                                <div class="col-md-12 ">
                                    <a href="{{ route('handleSocialLogin', ['customer', 'google']) }}" class="btn btn-outline-secondary btn-block"><i class="fab fa-google"></i> Google</a>
                                </div>
                            </div>
                        </form>
                        <!-- END Sign Up Form -->
                        <div class="col-md-12" style="text-align: left">
                            <a class="font-size-sm text-smirrow" href="{{ route('register') }}"><i class="fa fa fa-reply"> </i> {{ __('generics.return') }}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    <div class="block-header bg-primary-dark">
                        <h3 class="block-title">@lang('messages.terms_conditions') - SPEEHO</h3>
                    ...
                    <div class="block-content block-content-full text-right border-top">
                        <button type="button" class="btn btn-sm btn-link mr-2" data-dismiss="modal">@lang('generics.close')</button>
                        <button type="button" class="btn btn-sm btn-smirrow" data-dismiss="modal" onclick="agree()">@lang('messages.i_agree')</button>
                    </div>
                </div>
            </div>
        </div>
